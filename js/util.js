var util = {
	...
	changeSubpage: function(targetPage, activePage, aniShow) {
		...
		if(!localStorage.getItem('token')) {
			...
			plus.webview.show(plus.webview.getLaunchWebview())
			return
		}
		...
		if(mui.os.ios || aniShow[targetPage]) {
			...
			mui.extend(aniShow, temp);
			plus.webview.show(targetPage, "fade-in", 500);
		}
	},
	/**
	 * 点击重绘底部tab （view控件）
	...
	updateSubNView: function(currIndex, color) {
		...
		var self = plus.webview.currentWebview(),
			nviewEvent = plus.nativeObj.View.getViewById("AdDevicetabBar"), // 获取nview控件对象
	 ...
	 * 点击底部tab时，进行定位处理
	 */
	changeWindows:function(clientX){
		// 创建子webview窗口 并初始化
		var aniShow = {};
